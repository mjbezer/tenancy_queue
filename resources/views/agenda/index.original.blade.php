...
<div class="content">
    <div class="block">
        ...
        <div class="block-content pb-4">
            ...
            <hr>
            <table id="simple-datatable" class="table responsive table-hover table-striped nowrap" width="100%">
                <thead class="bg-earth-lighter">
                    <tr>
                        ...
                        <td class="text-center no-sort" style="min-width: 65px"><i class="si si-settings"></i></td>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($agendas as $agenda)
                    <tr>
                        <td class="text-left">{{ $agenda->inicio  }}</td>
                        <td>{{ $agenda->pedido->pessoa->nome_razao_social }}
                            (<b>{{ $agenda->pedido->pessoa->nome_usual_fantasia }}</b> )</td>
                        <td class="text-left">{{ $agenda->servico->descricao }}</td>
                        <td class="text-center">{{ $agenda->status_agenda ? 'Agendado' : 'Atendido' }}</td>
                        <td class="text-center">
                            {{ $agenda->pedido->contaReceber['data_pagamento'] ? 'Pago' : 'Aberto' }}</td>
                        <td class="text-center">
                            <a class="h4" data-toggle="tooltip" href="{{ route('agendas.edit', $agenda->id) }}"
                                title="Edita/Altera este registro...">
                                <i class="si si-note"></i>
                            </a>
                            <a class="h4 text-danger" href="javascript:deleteAgenda({{ $agenda->id }})">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Inserir Agenda</h5>
                    ...
                    <form id="formAgendamento" method="post" action="{{ route('agendas.store') }}" class="agenda">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6 mt-2">
                                ...
                                <select class="js-select2 form-control" style="width: 100%"
                                    name="pessoa_id" id="pessoas" required data-placeholder="Selecione um cliente ...">
                                </select>
                            </div>
        ...
        let calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            select: function (start) {
                ...
                if(start.startStr > agora){
                    ...
                    Swal.fire(
                        'Cuidado!',
                        'Data e hora selecionada menor que atual!',
                        'warning'
                        )
                }
            },
                ...
                $.ajax({
                    ...
                    .done(function (dados) {
                        ...
                        dados.forEach(function (agenda, key) {
                            eventos.push({
                                id: agenda.id,
                                title: agenda.servico.descricao + " - " + agenda
                                    .pedido.pessoa.nome_razao_social,
                                start: agenda.inicioJS,
                                end: agenda.fimJS,
                                textColor: agenda.servico.cor_texto,
                                backgroundColor: agenda.servico.cor_fundo,
